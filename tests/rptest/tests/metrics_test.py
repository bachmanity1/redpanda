...
class MetricsTest(Test):
    ...
    def start_redpanda(self, aggregate_metrics, enable_ts=False):
    ...
    def test_aggregate_metrics(self, aggregate_metrics):
        ...
        def filter_metrics(metrics):
            return list(metric for metric in metrics
                        if self.filter_metrics(metric))
    ...
    def _test_new_metrics(self, aggregate_metrics, public_metrics):
        ...
        with open(golden_metrics_path(aggregate_metrics,
                                      public_metrics)) as metrics_file:
            golden_metrics = self.clean_metrics(
                metrics_file.read().split("\n"))
    ...
    def test_regen_golden_metrics(self, generate_golden):
        ...
        def gen_metrics_file(aggregate_metrics, public_metrics):
            ...
            with open(golden_metrics_path(aggregate_metrics, public_metrics),
                      'w') as metrics_file:
                metrics_file.write('\n'.join(metrics))
